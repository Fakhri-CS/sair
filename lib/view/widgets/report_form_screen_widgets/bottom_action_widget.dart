    ...
    ref.listen<AsyncValue<ReportModel?>>(submitReportViewModelProvider, (previous, next) {
      next.when(
        data: (report) {
          if (report != null) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(l10n.reportSubmitSuccess)),
            );
        ...
        error: (error, stack) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(l10n.reportSubmitError(error.toString())),
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          onPressed: submitState.isLoading 
              ? null 
              : () => ref.read(submitReportViewModelProvider.notifier).submitReport(),
